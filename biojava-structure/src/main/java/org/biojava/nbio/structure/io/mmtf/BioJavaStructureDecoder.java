...
public class BioJavaStructureDecoder implements StructureDecoderInterface, Serializable {
	...
	public void cleanUpStructure() {
		...
		// And that micro-heterogenious groups are in the same groups
		for (int i =0; i< structure.nrModels() ; i++){
			for (Chain chain : structure.getModel(i)) {
				List<Group> removeGroups = new ArrayList<>();
				int oldResidueNumber = -1;
				Group oldGroup = chain.getAtomGroups().get(0);
				for (Group group : chain.getAtomGroups()) {
					// If this group has the same residue number as the last - it is actually an alt loc
					if (group.getResidueNumber().getSeqNum().equals(oldResidueNumber)) {
						oldGroup.getAltLocs().add(group);
						removeGroups.add(group);
					}
					oldResidueNumber = group.getResidueNumber().getSeqNum(); 
					oldGroup = group;
				}
				// Now remove these groups
				chain.getAtomGroups().removeAll(removeGroups);
				// Now sort out the alt locs
				for (Group group : chain.getAtomGroups()) {
					for (Group altLocGroup : group.getAltLocs()) { 
						for ( Atom groupAtom : group.getAtoms()) {
							// If this alt loc doesn't have this atom (and it's not micro heterogeneity)
